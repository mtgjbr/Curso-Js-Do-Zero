...
function recebeEventoForm(evento) {
    ...
    console.log(valor) // recebe o bõtao que foi apertado e define o valor com base na propriadade do dataset-valor
    return handleEvento(valor)
}
...
function fazConta() { // não posso só chamar o valor como parametro porque ele assim que a cada numero que eu apertar ele muda
    if (operacao === "+") {
    ...
    switch (valor) { //valor que vem do buttom sempre vem em string, por isso deve-se deixar o numero como string tbm
        case '+':
            ...
            if (numeroAnterior !== "") {
                ...
                ResultadoExibido = [numeroAtual]; // o numero anterior vira o atual e o anterior é limpo
            } numeroAnterior = numeroAtual;
            exibirResultado("+");
            operacao = valor
            numeroAtual = "";  //limpar o numero atual para o proximo calculo ele entrar como um novo
            
            break
        case '-':
            if (numeroAnterior !== "") {
                fazConta(operacao); //  && operacao === "subtracao" essa parte não é mais necessária porque a função conta ja verifica onde colocar com base na operacao
        ...
        case "X":
            if (numeroAnterior !== "") {
                fazConta(operacao); //ifs com apenas uma linha não precisam de {}
                ResultadoExibido = [numeroAtual]
            } numeroAnterior = numeroAtual;
            exibirResultado("X");
            operacao = valor;
